...
int vtysh_reconnect(struct vtysh_client *vclient);
...
// 返回字符串中第 n 个单词的起始指针
static const char* get_nth_word_position(const char* str, int n) {
    if (str == NULL || n <= 0) {
        return NULL;
    }
...
static int vtysh_execute_func(const char *line, int pager)
{
	...
	while (ret != CMD_SUCCESS && ret != CMD_SUCCESS_DAEMON
	       ...
	       && vty->node > CONFIG_NODE && vty->node != CONFIG_DEBUG_NODE) {
	...
	case CMD_ERR_NO_MATCH:
		vty_out(vty, "%% Unknown commandd: %s\n", line);
		break;
	...
	case CMD_SUCCESS_DAEMON: {
		...
		{
			if(!strncmp(cmd->string, "interface", sizeof("interface")-1)) {
				const char *ifp = get_nth_word_position(line, 2); /* Avoid abbreviation situations */
				if(ifp){
					char cmd[256];
					snprintf(cmd, sizeof(cmd), "ip a |grep BROADCAST |grep %s: >/dev/null 2>&1", ifp);
					if(system(cmd)){
						vty_out(vty,"no such interface:%s\r\n",ifp);
						cmd_stat = CMD_ERR_NO_FILE;
						break;
					
}
					snprintf(vty->ifname, sizeof(vty->ifname), "%s", ifp);
				}
			}
			else if(!strncmp(cmd->string, "address", sizeof("address")-1)) {
				const char *ifp = get_nth_word_position(line, 2);///address
				if(ifp){
					snprintf(vty->hy_node_context_name, sizeof(vty->hy_node_context_name), "%s", ifp);
				}
			}
			else if(!strncmp(cmd->string, "whitelist url", sizeof("whitelist url")-1)) {
				const char *ifp = get_nth_word_position(line, 3);///WHITELIST_URL_NODE
				if(ifp){
					snprintf(vty->hy_node_context_name, sizeof(vty->hy_node_context_name), "%s", ifp);
				}
			}
...
int vtysh_mark_file(const char *filename)
{
	...
	while (fgets(vty->buf, VTY_BUFSIZ, confp)) {
		...
		case CMD_ERR_NO_MATCH:
			fprintf(stderr, "line %d: %% Unknown commanda: %s\n",
				lineno, vty->buf);
			fclose(confp);
			vty_close(vty);
			XFREE(MTYPE_VTYSH_CMD, vty_buf_copy);
...
int vtysh_config_from_file(struct vty *vty, FILE *fp)
{
	...
	while (fgets(vty->buf, VTY_BUFSIZ, fp)) {
		...
		case CMD_ERR_AMBIGUOUS:
			...
			break;
		case CMD_ERR_NO_MATCH:
			fprintf(stderr, "line %d: %% Unknown commandb[%d]: %s",
				lineno, vty->node, vty->buf);
...
DEFUNSH_HIDDEN(VTYSH_LDPD, ldp_member_pseudowire_ifname,
...
DEFUNSH_HIDDEN(VTYSH_ISISD, isis_srv6_enable, isis_srv6_enable_cmd,
	"segment-routing srv6",
	SR_STR
...
DEFUNSH_HIDDEN(VTYSH_SR, segment_routing, segment_routing_cmd,
	"segment-routing",
...
DEFUNSH_HIDDEN(VTYSH_AFFMAP, affinity_map, vtysh_affinity_map_cmd,
...
/* max value is EXT_ADMIN_GROUP_MAX_POSITIONS - 1 */
DEFUNSH_HIDDEN(VTYSH_AFFMAP, no_affinity_map, vtysh_no_affinity_map_cmd,
...
DEFUNSH_HIDDEN(VTYSH_REALLYALL, vtysh_disable, vtysh_disable_cmd, "disable",
...
*/
    ...
    {	
    	case ADDRESS_NODE:
		case SCHEDULE_ABSOLUTE_NODE:
	      vtysh_execute("end");
	      vtysh_execute("configure terminal");
	      vty->node = CONFIG_NODE;
		  break;
    	case CONFIG_DEBUG_NODE:
		  vty->node = CONFIG_NODE;
		  break;
    	case TROUBLESHOOTING_NODE:
		  vty->node = ENABLE_NODE;
		  break;
		case ENABLE_NODE:
...
DEFUNSH_HIDDEN(VTYSH_ZEBRA, vtysh_pseudowire, vtysh_pseudowire_cmd,
	"pseudowire IFNAME",
...
DEFUNSH_HIDDEN(VTYSH_ALL, vtysh_allow_reserved_ranges, vtysh_allow_reserved_ranges_cmd,
...
DEFUNSH_HIDDEN(VTYSH_ALL, no_vtysh_allow_reserved_ranges,
...
DEFUN_HIDDEN (vtysh_integrated_config,
...
DEFUN_HIDDEN (no_vtysh_integrated_config,
...
void execute_command(const char *command, int argc, const char *arg1,
			    const char *arg2)
{
	pid_t pid;
	int status;
...
static struct cmd_node address_node = {
	.name = "address_node",
	.node = ADDRESS_NODE,
	.parent_node = CONFIG_NODE,
	.prompt = "%s(config-address)# ",
};

DEFUNSH(VTYSH_ALL,address, 
			address_cmd,
	"address NAME",
	"Config address\n"
	"Address's name\n")
{
	vty->node = ADDRESS_NODE;
	return CMD_SUCCESS;
}

static struct cmd_node whitelist_url_node = {
	.name = "whitelist_url_node",
	.node = WHITELIST_URL_NODE,
	.parent_node = CONFIG_NODE,
	.prompt = "%s(config-whitelist-url)# ",
};

DEFUNSH(VTYSH_ALL,whitelist_url, 
			whitelist_url_cmd,
	"whitelist url NAME",
	"Config whitelist\n"
	"Config url whitelist\n"
	"Url whitelist's name\n")
{
	vty->node = WHITELIST_URL_NODE;
	return CMD_SUCCESS;
}
...
void vtysh_init_vty(void)
{
	...
	//install_element(CONFIG_NODE, &vtysh_show_running_config_cmd);
#if 0
...
#if 0
	install_node(&address_node);
	install_element(CONFIG_NODE, &address_cmd);
	install_element(ADDRESS_NODE, &vtysh_exit_all_cmd);
	install_element(ADDRESS_NODE, &vtysh_end_all_cmd);
	install_element(ADDRESS_NODE, &vtysh_show_running_config_cmd);
	install_element(ADDRESS_NODE, &vtysh_quit_all_cmd);	
	
	install_node(&whitelist_url_node);
	install_element(CONFIG_NODE, &whitelist_url_cmd);
	install_element(WHITELIST_URL_NODE, &vtysh_exit_all_cmd);
	install_element(WHITELIST_URL_NODE, &vtysh_end_all_cmd);
	install_element(WHITELIST_URL_NODE, &vtysh_quit_all_cmd);	
	install_element(WHITELIST_URL_NODE, &vtysh_show_running_config_cmd);
#endif
	vtysh_hy_system_cmd_init();
	...
	//install_element(CONFIG_DEBUG_NODE, &vtysh_ping_cmd);
	//install_element(CONFIG_DEBUG_NODE, &vtysh_ping_ip_cmd);
	//install_element(CONFIG_DEBUG_NODE, &vtysh_traceroute_cmd);
	//install_element(CONFIG_DEBUG_NODE, &vtysh_traceroute_ip_cmd);
	//install_element(CONFIG_DEBUG_NODE, &vtysh_ping6_cmd);
	//install_element(CONFIG_DEBUG_NODE, &vtysh_traceroute6_cmd);
