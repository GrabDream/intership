...
#include <sys/ipc.h>
#include <sys/shm.h>
#define IF_NAME_LEN_MAX 32

#ifndef HYTF_CACHE_ALIGN_BYTE
#define HYTF_CACHE_ALIGN_BYTE     (64L)
#endif
typedef struct vpp_cpu_perthread_stat_struct
{
	unsigned int poll_totals[2];
	unsigned int poll_idles[2];
	unsigned int poll_cursor;
}__attribute__ ((aligned (HYTF_CACHE_ALIGN_BYTE)))VPP_CPU_PERTHREAD_STAT_T;

typedef struct vpp_if_info_struct
{
	char if_name[IF_NAME_LEN_MAX];
	unsigned int if_up:1,if_full_duplex:1,if_autoneg:1,if_mtu:14,reserved:15;
	unsigned int if_speed;
}VPP_IF_INFO_T;

typedef struct vpp_if_perthread_stat_struct
{
	unsigned long long rx_bytes;
	unsigned long long tx_bytes;
}VPP_IF_PERTHREAD_STAT_T;

typedef struct vpp_ifcpus_stat_struct
{
	unsigned int vpp_cpu_num;
	unsigned int vpp_ifs_num;
	unsigned char pading[HYTF_CACHE_ALIGN_BYTE - (sizeof(unsigned int)*2)];
	VPP_CPU_PERTHREAD_STAT_T cpu_stat[0];
}__attribute__ ((aligned (HYTF_CACHE_ALIGN_BYTE)))VPP_IFCPUS_STAT_T;

#define VPP_CPU_NUM(p) (((VPP_IFCPUS_STAT_T*)(p))->vpp_cpu_num)
#define VPP_IFS_NUM(p) (((VPP_IFCPUS_STAT_T*)(p))->vpp_ifs_num)
#define P_VPP_CPU_STAT(p, thread_index) ((thread_index >= VPP_CPU_NUM(p))?NULL:((((VPP_IFCPUS_STAT_T*)(p))->cpu_stat)+thread_index))
#define P_VPP_IF_INFO(p, sw_if_index) ((sw_if_index >= VPP_IFS_NUM(p))?NULL:(((VPP_IF_INFO_T*)((((VPP_IFCPUS_STAT_T*)(p))->cpu_stat)+VPP_CPU_NUM(p)))+sw_if_index))
#define P_VPP_IF_STAT(p, thread_index, sw_if_index) (((thread_index >= VPP_CPU_NUM(p)) || (sw_if_index >= VPP_IFS_NUM(p))) ? NULL : ((VPP_IF_PERTHREAD_STAT_T*)(((VPP_IF_INFO_T*)((((VPP_IFCPUS_STAT_T*)(p))->cpu_stat)+VPP_CPU_NUM(p)))+VPP_IFS_NUM(p)))+VPP_IFS_NUM(p)*thread_index+sw_if_index)
...
//zhanghuanan for vpp-23.02 version:
//#ifdef HYLAB_SUM
#define MAX_NETNUM 100
    static void* p_vpp_shm_ptr = NULL;
    if (p_vpp_shm_ptr == NULL) {
...
//#endif
    ...
    while (fgets(line, sizeof(line), devin)) {
        ...
        *stats++ = 0; /* null terminate name */
		...
		//#ifdef HYLAB_SUM
        if (p_vpp_shm_ptr != NULL) {
            unsigned int v_ifs = VPP_IFS_NUM(p_vpp_shm_ptr);
            unsigned int v_cpus = VPP_CPU_NUM(p_vpp_shm_ptr);

            for (unsigned int s = 0; s < v_ifs; s++) {
                VPP_IF_INFO_T* v_info = P_VPP_IF_INFO(p_vpp_shm_ptr, s);

                if (v_info && v_info->if_name[0] && strcmp(ifstart, v_info->if_name) == 0) {

                    entry->speed = v_info->if_speed;
                    entry->mtu = v_info->if_mtu;
                    entry->oper_status = v_info->if_up ? IFOPERSTATUS_UP : IFOPERSTATUS_DOWN;

                    unsigned long long rx_all = 0, tx_all = 0;
                    for (unsigned int c = 0; c < v_cpus; c++) {
                        VPP_IF_PERTHREAD_STAT_T* v_stat = P_VPP_IF_STAT(p_vpp_shm_ptr, c, s);
                        if (v_stat) {
                            rx_all += v_stat->rx_bytes;
                            tx_all += v_stat->tx_bytes;
                        }
                    }

                    entry->stats.ibytes.low = rx_all & 0xffffffff;
                    entry->stats.ibytes.high = rx_all >> 32;
                    entry->stats.obytes.low = tx_all & 0xffffffff;
                    entry->stats.obytes.high = tx_all >> 32;
